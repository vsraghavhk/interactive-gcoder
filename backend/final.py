...
class Model:
    ...
    def __init__(self, file=data_file):
        ...
        global filename 
        filename = "gcodes/func{}_{}sides_{}mmEdge.gcode".format(self.func_choice, self.num_sides, self.edge_length)
    ...
    def straight_xy(self, x, y, cx = -1, cy = -1):
        ...
        if cx==-1 and cy==-1: 
            ...
            for i in range(0, self.num_sides):
                x.append(round(cx + self.radius * math.cos(2*math.pi*i/self.num_sides), 2))
                y.append(round(cy + self.radius * math.sin(2*math.pi*i/self.num_sides), 2))  
    ...
    def spiral_xy(self, x, y, cx = -1, cy = -1):     
        ...
        if cx==-1 and cy==-1:
            ...
            for i in range(0, self.num_sides):
                x.append(round(cx + self.radius * math.cos((2*math.pi*i/self.num_sides)+(self.cur_layer*0.0174533)), 2))
                y.append(round(cy + self.radius * math.sin((2*math.pi*i/self.num_sides)+(self.cur_layer*0.0174533)), 2))
        return x, y
    ...
    def random_xy(self, x, y):
        self.radius = self.radius + (randrange(self.random_value) - self.random_value/2)/10
        self.calc_rad_el()
        return self.straight_xy(x, y)
